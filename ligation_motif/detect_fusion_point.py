...
#				f2_seq = genome[f2_point-settings.width:f2_point+1].reverse_complement()
			...
			acc1 = computeACC((s1+overlap+s2)[:side_5p_len], winlen=1)
			acc2 = computeACC((s1+overlap+s2)[-side_3p_len:], winlen=1)

			# if len(acc1) != len(f1_seq):
			# 	print "unmatched in first", 
			# 	print (s1+overlap)[:side_5p_len], "|||||", f1_seq
			# if len(acc2) != len(f2_seq):
			# 	print "unmatched in second", len(overlap+s2), side_3p_len
			# 	print (overlap+s2)[-side_3p_len:], "|||||", f2_seq
			# 	print read_3ps[rname][1]
			# 	if read_3ps[rname][1] == "-":
			# 		print genome[read_3ps[rname][0]:read_3ps[rname][0] + side_3p_len - 1 + 1].reverse_complement()
			# 	else:
			# 		print genome[read_3ps[rname][0] - (side_3p_len - 1):read_3ps[rname][0] + 1]

			# if len(acc1) != len(f1_seq) or len(acc2) != len(f2_seq):
			# 	print "sequence to split:", s1 + overlap + s2
			# 	print s1, len(s1), side_5p_len
			# 	print overlap, len(overlap)
			# 	print s2, len(s2), side_3p_len
			# 	print "r1_seq", r1_seqs[rname]
			# 	print "r2_seq", r2_seqs[rname]
			# 	print "read name", rname
			# 	print "-" * 50
