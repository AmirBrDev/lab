...
class TableLoader(object):
	...
	def load(self, path):
		...
		name_list = fl.readline().lower().replace("\n", "").replace("\r", "").split(Table.TABLE_DELIMITER)

		for line_number, line in enumerate(fl.readlines()):
			...
			if (len(name_list) != len(value_list)):
				print name_list
				print value_list
				raise StructureException("Unmatched arguments count in table. line %d" % line_number)
...
class McDowellTableLoader(TableLoader):
	...
	def loadUnprocessed(self, path):
		...
		for entry in result:
			...
			entry["name"] = "_".join([entry["chrom_pos"], entry["strand"]])

			entry[TableGlobals.FIRST_START_BASE_KEY] = entry.pop('chrom_pos')
			entry[TableGlobals.FIRST_END_BASE_KEY] = entry[TableGlobals.FIRST_START_BASE_KEY]
			entry[TableGlobals.FIRST_STRAND_KEY] = entry.pop('strand')

			if (entry[TableGlobals.FIRST_STRAND_KEY] == "fwd"):
				entry[TableGlobals.FIRST_STRAND_KEY] = "+"
			else:
				entry[TableGlobals.FIRST_STRAND_KEY] = "-"
